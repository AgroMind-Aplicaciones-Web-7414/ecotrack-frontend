...
// 🔥 función para limpiar URLs (soluciona el doble slash)
...
export class TaskApi {
    ...
    async getTasksCompleted() {
        const endpoint = buildUrl(this.baseUrl, `${this.tasksEndpoint}?status=Completed`);
        try {
    ...
    async getTasksInProgress() {
        const endpoint = buildUrl(this.baseUrl, `${this.tasksEndpoint}?status=InProgress`);
        try {
            const response = await axios.get(endpoint);
            return response.data.map(task => TaskAssembler.toEntityFromResponse(task));
        } catch (error) {
    ...
    async getTasksPending() {
        const endpoint = buildUrl(this.baseUrl, `${this.tasksEndpoint}?status=Pending`);
        try {
            const response = await axios.get(endpoint);
            return response.data.map(task => TaskAssembler.toEntityFromResponse(task));
        } catch (error) {
    ...
    async updateStatus(taskID, newStatus) {
        const endpoint = buildUrl(this.baseUrl, `${this.tasksEndpoint}/${taskID}/status`);
        try {
    ...
    async updateCompletedDate(taskID, newCompletedDate) {
        const endpoint = buildUrl(this.baseUrl, `${this.tasksEndpoint}/${taskID}`);
        try {
    ...
    async registerNewTask(resource) {
        try {
            const endpoint = buildUrl(this.baseUrl, this.tasksEndpoint);
